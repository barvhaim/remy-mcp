...
class TestIsraeliLandAPIE2E:
    ...
    @pytest.mark.e2e
    @pytest.mark.api
    def test_search_by_tender_types(
        self, api_client, rate_limiter, test_data_validator
    ):
    ...
    def test_search_with_date_range(
        ...
        results = api_client.search_tenders(
            publication_date_from=date_from, page_size=5
        )
    ...
    @pytest.mark.e2e
    @pytest.mark.api
    def test_search_with_settlement_code(
        self, api_client, rate_limiter, test_data_validator, known_settlement_codes
    ):
        ...
        results = api_client.search_tenders(kod_yeshuv=settlement_code, page_size=5)
    ...
    @pytest.mark.e2e
    @pytest.mark.api
    def test_get_active_tenders_method(
        self, api_client, rate_limiter, test_data_validator
    ):
    ...
    @pytest.mark.e2e
    @pytest.mark.api
    def test_search_by_location_method(
        self, api_client, rate_limiter, test_data_validator, known_settlement_codes
    ):
    ...
    def test_error_handling_invalid_tender_id(self, api_client, rate_limiter):
        ...
        if "MessageDetails" in result:
            ...
            assert (
                result.get("MichrazID") in [0, None]
                or result.get("MichrazName") is None
            )
    ...
    @pytest.mark.e2e
    @pytest.mark.api
    def test_comprehensive_search_filters(
        self, api_client, rate_limiter, test_data_validator
    ):
