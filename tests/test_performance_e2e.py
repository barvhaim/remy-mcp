...
class TestPerformanceE2E:
    ...
    def test_api_response_times(self):
        ...
        assert (
            stats["avg_response_time"] < 10.0
        ), f"Average response time too high: {stats['avg_response_time']}"
        assert (
            stats["max_response_time"] < 30.0
        ), f"Max response time too high: {stats['max_response_time']}"
    ...
    def test_details_endpoint_performance(self):
        ...
        assert (
            stats["avg_response_time"] < 8.0
        ), f"Details endpoint too slow: {stats['avg_response_time']}"
    ...
    def test_map_endpoint_performance(self):
        ...
        assert (
            stats["avg_response_time"] < 15.0
        ), f"Map endpoint too slow: {stats['avg_response_time']}"
    ...
    def test_search_with_complex_filters_performance(self):
        ...
        complex_searches = [
            ...
            {"regions": [4], "tender_statuses": [1], "page_size": 10},
        ]
        ...
        assert (
            stats["avg_response_time"] < 12.0
        ), f"Complex searches too slow: {stats['avg_response_time']}"
    ...
    def test_pagination_performance(self):
        ...
        for page_num in range(1, 4):  # Test first 3 pages
            with timed_api_call(tracker):
                results = api_client.search_tenders(page_size=10, page_number=page_num)
                assert results is not None
        ...
        if len(response_times) >= 3:
            ...
            assert (
                time_variance < avg_time * 0.5
            ), f"High pagination variance: {time_variance}"
    ...
    def test_sustained_load_performance(self):
        ...
        ), f"High error rate under load: {stats['error_rate']}"
        ...
        if len(response_times) >= 10:
            ...
            assert (
                degradation < 0.5
            ), f"Performance degraded under load: {degradation * 100}%"
